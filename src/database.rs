...
impl<'l> Database<'l> {
    ...
    #[inline]
    pub fn push(&mut self, value: &str) -> Result<(), Error> {
        // write string and string length
        self.stackdb.write(self.tail, &value.len().to_be_bytes())?;
        self.stackdb.write(self.tail+8, value.as_bytes())?;
    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self, Error> {
        let mut stackdb = StackDB::new(SkdbDirAlloc::load(path.as_ref().to_path_buf())?)?;
...
impl<'l> Iterator for Database<'l> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Ok(len) = self.stackdb.read(self.idx..self.idx+8) {
            ...
            if let Ok(string) = self.stackdb.read(self.idx+8..self.idx+8+len) {
                // update idx
                self.idx += 8 + len;
